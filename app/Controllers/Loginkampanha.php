...
class Loginkampanha extends BaseController
{
    public function index(): string
    {
        return view('auth/loginkampanha/loginkampanha');
    }
    ...
    public function login()
    {
        ...
        return view('auth/loginkompanha/loginkampanha');
    }

    public function processologinkampanha()
    {
        $data = $this->request->getPost();
        $query = $this->db->table('akundiresaun')->getWhere(['naran_kompleto_diresaun'=> $data['naran_kompleto_diresaun']]);
        $user = $query->getRow();
        if ($user) {
            if ($user->regulamento_diresaun == 5) {
                if ($user->valido == 1) {
    ...
    public function logout()
    {
        ...
        return redirect()->to(base_url('kampanha'));
    }
}
